...
public class About extends JFrame {
	...
	public About() {
		...
		table.setModel(new DefaultTableModel(
			new Object[][] {
				{"", ""},
				{" Storage Path", " This tab Provides Local or remote system storage path Like \"C:\\Users\\User1\\Results\" or \"network address\""},
				{null, " * Storage path is required and path must be valid"},
				{" Start button", " Validate the storage path, If the storage path is valid Start button will disappear and (+) & (-) button will be enabled"},
				{" (+) button", " Green color (+) button allows user to add TC name"},
				{null, " * Creates folder of given TC name if not exist before and overlaps the one if already exists with the same name."},
				{null, " * The newly added TC name radio button will appear in the application window"},
				{null, " * Sub TC Folder can be added in the below format :"},
				{null, "   [TC Folder Name\\Sub TC Folder Name] "},
				{" (-) button", " Red color (-) button removes selected TC name from the list but not the contents inside it"},
				...
				{" Zip", " Zip Button provides the list of folders and subfolders inside the Storage path directory to compress and zip it"},
				{null, " * The List shows only non-zipped and non-empty folders and subfolders"},
